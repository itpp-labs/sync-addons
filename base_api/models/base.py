...
class Base(models.AbstractModel):
    ...
    def search_read_nested(self, domain=None, fields=None, offset=0, limit=None, order=None, delimeter='/'):
        result = pinguin.get_dictlist_from_model(self._name, tuple(fields),
                                                 domain=domain, offset=offset,
                                                 limit=limit, order=order,
                                                 env=self.env, delimeter=delimeter)
        return result
    ...
    def create_or_update_by_external_id(self, vals):
        ...
        def convert_external_2_inner_id(ext_id):
            try:
                ...
                raise ValueError('No object with external id in field %s: %s' % (field, ext_id))
            return result
